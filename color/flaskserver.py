...
def login():
    ...
    if request.method == 'POST':
        ...
        if len(result) == 0:
            ...
            user = result[0][0]
            print(user)
            return redirect(url_for('.colors',error=error, user=user))
...
def colors():
    ...
    if request.method == 'POST':
        username = request.form['username']
        print(username)
        # comment1 = request.form.get('colour1')
        comment1 = request.form['colour1']
        comment2 = request.form['colour2']
        # comment1 = request.form.get('colour2')
        comment3 = request.form['colour3']
        # comment1 = request.form.get('colour3')
        print(888888)
        print(comment1)

        comment = '1 - '+comment1+'\n'+'2 - '+comment2+'\n'+'3 - '+comment3
        # get the hour last time submitted
        cursor.execute("SELECT whichday,time FROM colours WHERE username = %s", (username,) )
        ...
        try:
            ...
            if time:
                ...
                if yesterday == last_submit:
                    whichday += 1
                    sql = "UPDATE colours SET whichday=%s,time=%s,day{}=%s WHERE username = %s".format(whichday)
            ...
            # for the first day
                ...
                cursor.execute("INSERT into colours (username,whichday,time,day1,day2,day3,day4,day5,day6,day7) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            ...
            print(e)
            return render_template('colours.html',user=user,typo=type)

    else:
        user1 = request.args['user']
        user = {"username": user1}
    return render_template('colours.html', error=error, user=user)
...
#=========================below==================================================================
    ...
    sql4 ='''
            ...
            whichday INT DEFAULT 99,
            time VARCHAR(15) DEFAULT '',
            day1 VARCHAR(100) DEFAULT '',
            day2 VARCHAR(100) DEFAULT '',
            day3 VARCHAR(100) DEFAULT '',
            day4 VARCHAR(100) DEFAULT '',
            day5 VARCHAR(100) DEFAULT '',
            day6 VARCHAR(100) DEFAULT '',
            day7 VARCHAR(100) DEFAULT '')
            '''
    cursor.execute(sql4)
    print('sql5')
